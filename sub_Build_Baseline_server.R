################################################################################
# Events associated with the building of the baseline scenario.
################################################################################
observeEvent(input$add_BaselineScenario, {
  ...
  updateSwitchInput(session, inputId = "display_SpeciesParameters", value = FALSE)
  updateSwitchInput(session, inputId = "display_LifeHistory", value = FALSE)
  
  updateSwitchInput(session, inputId = "plot_all_on_off", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_growth", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_growth_trajectory", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_survival_trajectory", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_length_mass", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_survival", value = FALSE)
  updateSwitchInput(session, inputId = "plot_clear_species_reproduction", value = FALSE)
...
observeEvent(input$submit_name, {
  if (is.null(input$currentScenarioName) ||
      (trimws(input$currentScenarioName) == ""))
  {
...
observeEvent(input$baselines, {
  ...
  if ("survival_decrement" %in% colnames(parameters[[chosen_scenario_name]]))
  {
    output$underlying_scenario <- renderText({
      paste(
        "If a chemical stressor is chosen to overlay on a chemical stressor scenario,
...
observeEvent(input$plot_all_on_off, {
  if (input$plot_all_on_off)
  {
    if (input$species != "New")
    {
      updateSwitchInput(session, inputId = "plot_clear_species_growth", value = TRUE)
      updateSwitchInput(session, inputId = "plot_clear_species_growth_trajectory", value = TRUE)
      updateSwitchInput(session, inputId = "plot_clear_species_survival_trajectory", value = TRUE)
      updateSwitchInput(session, inputId = "plot_clear_species_length_mass", value = TRUE)
      updateSwitchInput(session, inputId = "plot_clear_species_survival", value = TRUE)
      updateSwitchInput(session, inputId = "plot_clear_species_reproduction", value = TRUE)
    } else
    {
      updateSwitchInput(session, inputId = "plot_clear_species_growth", value = FALSE)
      updateSwitchInput(session, inputId = "plot_clear_species_growth_trajectory", value = FALSE)
  ...
  if (input$plot_clear_species_growth)
  {
    if (input$species != "New")
    {
      output$Species_Growth_out <- renderPlot({
        index <- which(species_library$common_name == CurrentSpeciesName,
                       arr.ind = TRUE)
...
output$plotSpeciesGrowth <- renderPlot(
  FishToxTranslator::PlotGrowth(forProfile = TRUE, species = species_library$common_name[which(species_library$common_name == CurrentSpeciesName, arr.ind = TRUE)])
)
...
output$plotSpeciesGrowthTrajectory <- renderPlot(FishToxTranslator::PlotGrowthTrajectory(species = species_library$common_name[which(species_library$common_name == CurrentSpeciesName, arr.ind = TRUE)]))
...
output$plotSpeciesLengthMass <- renderPlot(FishToxTranslator::PlotLengthToMass(species = species_library$common_name[which(species_library$common_name == CurrentSpeciesName, arr.ind = TRUE)]))
...
observeEvent(input$upload_history_pars, {
  assign_history_pars(TemporaryBaselineScenarioName,
                      input$upload_history_pars$datapath)
  label_str <- paste("View complete ",
                     TemporaryBaselineScenarioName,
                     " parameters")
})

observeEvent(input$upload_scenario_pars, {
  assign_scenario_pars(TemporaryBaselineScenarioName,
                       input$upload_scenario_pars$name)
})
  ...
  } else
  {
    ...
    output$text_load_fhm <- renderText({
      paste(
        ...
        "'. ",
        CurrentSpeciesName,
        " parameters have been loaded into memory, and the ",
        "spawning algorithm for '",
        TemporaryBaselineScenarioName,
        "'",
        " has completed.",
        sep = ""
    ...
    enter_baseline_scenario_name(TemporaryBaselineScenarioName,
                                 input$textBaselineDescription)
    ...
    updateSelectInput(session, inputId = "stressor_type", selected = "None Selected")
    ...
    updateSwitchInput(session, inputId = "display_LifeHistory", value = TRUE)
    
    subElement1 <- paste("#Check_Scenario_Names_Results input[value=",
                         scenario_names,
                         "]")
    ...
    output$textMessageResetApp <- renderText({
      text1 <- paste(
        "The following scenarios and runs will be deleted upon clicking the Reset App button."
      )
...
# observeEvent(input$show_species_profile_modal, {
#   toggleModal(session, "species_profile_Popup", toggle = "open")
#   # Trigger the 'shown' event for the uiOutput element
#   runjs("$('#rmark').trigger('shown');")
# })
...
# output$baseline_mark <- shiny::renderUI({
#   #  if (generate_species_markdown() == TRUE)
#   #  {
#   path_rmd <- "Species_Profile.Rmd"
#   cat("path_rmd = ", path_rmd, "\n")
#   # Render into www folder.
#   system.name <- Sys.info()[["sysname"]]
#   fish_name <- sub(" ", "_", CurrentSpeciesName)
#   path_html <- ifelse(
...
################################################################################
# Functions used for the building of the baseline scenario.
################################################################################
...
# Baseline Name
#-------------------------------------------------------------------------------
enter_baseline_scenario_name <- function(currentScenarioName,
...
LifeHistory_Parameters <- function()
{
  ...
  lifeHistoryTemplate <- add_column(lifeHistoryTemplate, value = NA, .after =
                                      "id")
...
{
  chosenSpeciesDataObject <- as.character(species_library$parameter_data[which(species_library$common_name ==
                                                                                 chosenSpecies)])
...
run_spawning_algorithm <- function(currentScenarioName)
{
  ...
  dailySpawningProb <- GenerateSpawningProbs(
    repro_start = parameters[[currentScenarioName]]$repro_start[1],
  ...
  parameters[[currentScenarioName]] <<- cbind(parameters[[currentScenarioName]], p_spawn =
                                                dailySpawningProb)
}

assign_history_pars <- function(currentScenarioName, input_life_history)
{
  newLifeHistory <- read.csv(
    file = input_life_history,
    header = TRUE,
...
{
  return(get(species_library$parameter_data[which(species_library$common_name ==
                                                    chosenSpecies)]))
}
...
plot_spawning_probabilities <- function(currentScenarioName)
{
  ...
  plot(
    parameters[[currentScenarioName]]$p_spawn,
...
export_baseline_parameters <- function(scenario)
{
  ...
  file_name <- paste(scenario,
                     "_parameters_time",
                     gsub(":", "_", strsplit(date(), " ")[[1]][4]),
                     ".csv",
                     sep = "_")
