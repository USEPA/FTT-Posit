################################################################################
# Events for the handling of visualization of results.
################################################################################
...
observe({
  updateCheckboxGroupInput(
    ...
    choices = as.list(names(unlist(
      modelRuns, recursive = F
    ))),
    selected = if (input$All_Results)
      as.list(names(unlist(
        modelRuns, recursive = F
      )))
  )
...
observe({
  if (is.null(input$Check_Scenario_Names_Results))
  {
    ...
    updateSwitchInput(session, inputId = "plot_clear_summary_matrix", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_summary_results_table", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_dailyPopulation", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_populationBiomass", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_meanSize", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_growthPotential", value = FALSE)
    updateSwitchInput(session, inputId = "plot_clear_transitionalKernel", value = FALSE)
  } else if (length(input$Check_Scenario_Names_Results) == 1)
  {
    ...
    {
      shinyjs::info(
        "You must select at least 2 scenarios. If only one scenario was simulated and as a result only one is active, you won't be able to display a summary matrix."
      )
...
output$mySummaryResultsTable <- DT::renderDataTable({
  return_summary_results_table(input$Check_Scenario_Names_Results)
}, options = list(scrollX = TRUE))

plotSummaryResultsTableModal <- function() {
  modalDialog(
    title = "",
    DT::dataTableOutput("mySummaryResultsTable"),
    size = "l",
    footer = tagList(
      dSRT_button,
      modalButton("Close")
...
observeEvent(input$export_summaryResults_modal, {
  ...
  {
    showModal(plotSummaryResultsTableModal())
  }
})
...
#       paste(
#         "Please select two or more scenarios before attempting to export a plot.",
#         "To close this window click the 'Close' button below."
#       )
#     })
...
#     output$textMessageSummaryMatrix <- renderText({
#       paste(
#         "You must select at least 2 scenarios.",
...
output$mySummaryMatrix <- renderPlot(
  {
    plot_Summary_Matrix(input$Check_Scenario_Names_Results)
  }
)

plotSummaryMatrixModal <- function() {
  modalDialog(
    title = "",
    plotOutput("mySummaryMatrix", width = "100vh", height = "100vh"),
    size = "l",
    footer = tagList(
      dSM_button,
      modalButton("Close")
...
observeEvent(input$export_summaryMatrix_modal, {
  ...
  {
    summaryMatrix_flag <- FALSE
    shinyjs::info("You must select at least 2 scenarios. If only one scenario was simulated and as a result only one is active, you won't be able to display a results matrix.")
  } else
  {
    summaryMatrix_flag <- TRUE
    showModal(plotSummaryMatrixModal())
  }
})
...
#       paste(
#         "Please select one or more scenarios before attempting to export table.",
#         "To close this window click the 'Close' button below."
#       )
#     })
...
output$mySummaryMatrixTable <- DT::renderDataTable(
  {
    as.data.frame(SummaryMatrix(modelOutputs))
  }, options = list(scrollX = TRUE))

plotSummaryMatrixTableModal <- function() {
  modalDialog(
    title = "",
    DT::dataTableOutput("mySummaryMatrixTable"),
    size = "l",
    footer = tagList(
      dSMT_button,
      modalButton("Close")
...
observeEvent(input$export_summaryMatrixTable_modal, {
  if (is.null(input$Check_Scenario_Names_Results))
  {
    shinyjs::info("Please select one or more scenarios before attempting to export a table.")
  }else if (length(input$Check_Scenario_Names_Results) < 2)
  {
    shinyjs::info("You must select at least 2 scenarios. If only one scenario was simulated and as a result only one is active, you won't be able to display a results matrix table.")
  } else
  {
    showModal(plotSummaryMatrixTableModal())
  }
})
...
#     output$plotDailyPopulation <- renderPlot(plot_Daily_Population(input$Check_Scenario_Names_Results))
#   }
# })

output$myDailyPopulation <- renderPlot(
  {
    plot_Daily_Population(input$Check_Scenario_Names_Results)
  }
)

plotDailyPopulationModal <- function() {
  modalDialog(
    title = "",
    plotOutput("myDailyPopulation"),
    size = "l",
    footer = tagList(
      dDP_button,
      modalButton("Close")
...
observeEvent(input$export_dailyPopulation_modal, {
  ...
  {
    showModal(plotDailyPopulationModal())
  }
})
...
#     output$plotPopulationBiomass <- renderPlot(plot_Population_Biomass(input$Check_Scenario_Names_Results))
#   }
# })

output$myPopulationBiomass <- renderPlot(
  {
    plot_Population_Biomass(input$Check_Scenario_Names_Results)
  }
)

plotPopulationBiomassModal <- function() {
  modalDialog(
    title = "",
    plotOutput("myPopulationBiomass"),
    size = "l",
    footer = tagList(
      dPB_button,
      modalButton("Close")
...
observeEvent(input$export_populationBiomass_modal, {
  ...
  {
    showModal(plotPopulationBiomassModal())
  }
})
...
#     output$plotMeanSize <- renderPlot(plot_Mean_Size(input$Check_Scenario_Names_Results))
#   }
# })

output$myMeanSize <- renderPlot(
  {
    plot_Mean_Size(input$Check_Scenario_Names_Results)
  }
)

plotMeanSizeModal <- function() {
  modalDialog(
    title = "",
    plotOutput("myMeanSize"),
    size = "l",
    footer = tagList(
      dMS_button,
      modalButton("Close")
...
#     output$plotGrowthPotential <- renderPlot(plot_Growth_Potential(input$Check_Scenario_Names_Results))
#   }
# })

output$myGrowthPotential <- renderPlot(
  {
    plot_Growth_Potential(input$Check_Scenario_Names_Results)
  }
)

plotGrowthPotentialModal <- function() {
  modalDialog(
    title = "",
    plotOutput("myGrowthPotential"),
    size = "l",
    footer = tagList(
      dGP_button,
      modalButton("Close")
...
observeEvent(input$export_growthPotential_modal, {
  ...
  {
    showModal(plotGrowthPotentialModal())
  }
})
...
observeEvent(input$export_transitionalKernel_modal, {
  ...
  } else
  {
    ...
    output$plotTransitionalKernel <- renderPlot(plot_Transitional_Kernel(input$Check_Scenario_Names_Results))
  }
})
...
output$Download_Results_Report <- downloadHandler(
  ...
  content = function(file)
  {
    ...
    wb <- wb_add_worksheet(wb, sheet = "Results_report_information")
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Results Report Information", start_col = 1, start_row = 1)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Scenarios:", start_col = 1, start_row = 2)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Descriptions:", start_col = 1, start_row = 3)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Report Date/Time:", start_col = 1, start_row = 4)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Report contents", start_col = 1, start_row = 5)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Fish Toxicity Translator Version:", start_col = 1, start_row = 6)
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "For more information visit:", start_col = 1, start_row = 7)
    ...
    for (i in 1:len_list1)
    {
      ...
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = list1[[i]], start_col = j, start_row = 2)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = scenario_desc, start_col = j, start_row = 3)
    }
    
    # Date of report
    # xlsx::setCellValue(cells[[4, 2]], date())
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = date(), start_col = 2, start_row = 4)
    
    # Report contents
    # xlsx::setCellValue(cells[[5, 2]], "Summary_Results_Table")
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "Summary_Results_Table", start_col = 2, start_row = 5)
    ...
    if (length(input$Check_Scenario_Names_Results) >= 2)
    {
      ...
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Summary_Matrix", start_col = 3, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Summary_Matrix_Table", start_col = 4, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Daily_Population_Image", start_col = 5, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Population_Biomass_Image", start_col = 6, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Mean_Size_Image", start_col = 7, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Growth_Potential_Image", start_col = 8, start_row = 5)
    } else{
      ...
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Daily_Population_Image", start_col = 3, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Population_Biomass_Image", start_col = 4, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Mean_Size_Image", start_col = 5, start_row = 5)
      wb <- wb_add_data(wb, sheet = "Results_report_information", 
                  x = "Growth_Potential_Image", start_col = 6, start_row = 5)
    }
    ...
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = as.character(packageVersion("FishToxTranslator")), start_col = 2, start_row = 6)
    
    # Website
    # xlsx::setCellValue(cells[[7, 2]], "<link to website>")
    wb <- wb_add_data(wb, sheet = "Results_report_information", 
                x = "<link to website>", start_col = 2, start_row = 7)
    wb <- wb_set_col_widths(
      wb,
      sheet = "Results_report_information",
      cols = 1:8,
      widths = "auto",
      hidden = FALSE
    )
    ...
    summaryTable <- as.data.frame(SummaryTable(modelOutputs))
    wb <- wb_add_worksheet(wb, sheet = "Summary_Results_Table")
    wb <- wb_add_data_table(wb, sheet = "Summary_Results_Table", 
                      x = summaryTable, start_col = 1, start_row = 1)
    wb <- wb_set_col_widths(
      wb,
      sheet = "Summary_Results_Table",
      cols = 1:12,
      widths = "auto",
      hidden = FALSE
    )
    ...
    # 
     ...
     if (length(input$Check_Scenario_Names_Results) >= 2)
     {
       ...
       wb <- wb_add_worksheet(wb, sheet = "Summary_Matrix")
       image_path1 <- tempfile(pattern = "", fileext = ".png")
       png(
         image_path1,
         width = input$shiny_width * 8,
         height = input$shiny_height * 10,
         res = 1200
       )
       plot_Summary_Matrix(input$Check_Scenario_Names_Results)
       dev.off()
    ...
    #   )
     ...
     # Sheet 3: Summary Matrix Table
       ...
       wb <- wb_add_worksheet(wb, sheet = "Summary_Matrix_Table")
       
       wb <- wb_add_data_table(
         wb,
         sheet = "Summary_Matrix_Table", 
         x = summMats,
         with_filter = FALSE,
         start_col = 1, 
         start_row = 1)
    ...
    wb <- wb_add_worksheet(wb, sheet = "Daily_Population_Image")
    ...
    wb <- wb_add_worksheet(wb, sheet = "Population_Biomass_Image")
    ...
    wb <- wb_add_image(
      wb,
      sheet = "Population_Biomass_Image",
      dims = "D4",
      file = image_path3,
      width = 16,
      height = 8,
    ...
    wb <- wb_add_worksheet(wb, sheet = "Mean_Size_Image")
    ...
    wb <- wb_add_worksheet(wb, sheet = "Growth_Potential_Image")
...
#     path_html <- ifelse(
#       system.name == "Windows",
#       paste("www\\", "Results_Report.html", sep = ""),
#       paste("www/", "Results_Report.html", sep = "")
#     )
#     render(
#       path_rmd,
...
#-------------------------------------------------------------------------------
# Results markdown document
#-------------------------------------------------------------------------------
plotResultsMarkdownModal <- function() {
  modalDialog(
    title = "",
    tagList(
      {
...
observeEvent(input$export_results_markdown_modal, {
  if (is.null(input$Check_Scenario_Names_Results))
  {
    shinyjs::info("Please select one or more scenarios before attempting to export the results.")
  } else
  {
    showModal(plotResultsMarkdownModal())
  }
})
...
################################################################################
#  Functions used for the visualization of simulation results.
################################################################################
return_summary_results_table <- function(inputScenariosForResults)
{
  ...
  {
    summaryTableResults <- FishToxTranslator:::SummaryTable(unlist(modelRuns, recursive =
                                                                     F)[inputScenariosForResults])
