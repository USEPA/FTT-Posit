...
tabHome <- tabPanel("About",
                    div(img(src='ft_logov01beta.png', width=300, halign = "center"),style="text-align: center;"),
                      ...
                      h4('The Fish Toxicity Translator is a model that translates acute and chronic effects of
                    ...
                    h5(tags$li('The Fish Toxicity Translator works by introducing stressor scenarios to an underlying annual baseline life history model')),
                      ...
                      h5(tags$li('Non-chemical stressors can also be modeled and overlaid on baseline or chemical stressor scenarios.  The current non-chemical stressors included in the model are (1) Over-winter growth and survival and (2) Density-dependent growth effects')),
                      h5(tags$li('Model outputs include a variety of daily population metrics including:
                      ...
                      div(img(src='workflow_diagram_2.png', width=729, halign = "center"),style="text-align: center;"),
                      h2("Contact Information"),
                      h5("This beta version of the graphical user interface and model is undergoing continuous development.  Please do not hesitate to contact Nate Pollesch (pollesch.nathan@epa.gov) or Matthew Etterson (etterson.matthew@epa.gov) with any questions.")
                  )
                 ...
                 sidebarLayout
                    (
                      ...
                      (
                        shinyjs::hidden(
                          div(id = "Visualization_GRS",
                              ...
                              br(),
                              fluidRow(
                                column(width = 3, h4("Growth Functions", class = "header4")),
                                column(width = 4, offset = 1, plot_clear_growth_button),
                                column(width = 3, offset = 1, export_growth_functions_button)
                              ),
                              h4(""),
                              fluidRow(
                                column(width = 3, h4("Survival Functions", class = "header4")),
                                column(width = 4, offset = 1, plot_clear_survival_button),
                                column(width = 3, offset = 1, export_survival_functions_button)
                              ),
                              h4(""),
                              fluidRow(
                                column(width = 3, h4("Reproduction Functions", class = "header4")),
                                column(width = 4, offset = 1, plot_clear_reproduction_button),
                                column(width = 3, offset = 1, export_reproduction_functions_button)
                              ),
                              h4(""))
                        ),
                        shinyjs::hidden(
                          div(id = "Visualization_SPB",
                              ...
                              fluidRow(
                                ...
                                column(width = 4, offset = 1, plot_clear_spawning_probabilities_button),
                                column(width = 3, offset = 1, export_spawning_functions_button)
                              ),
                              h4(""))
                        ),
                          ...
                          div(id = "Visualization_SDEC",
                              shinyjs::hidden(div(id = "Show_SDEC",
                                                  fluidRow(
                                                    ...
                                                    column(width = 4, offset = 1, plot_clear_survival_decrements_button),
                                                    column(width = 3, offset = 1, export_survival_decrements_button)
                                                  ))),
                              h4(""),
                              shinyjs::hidden(div(id = "Show_GP",
                                                  fluidRow(
                                                    column(width = 3, h4("Growth Percents", class = "header4")),
                                                    column(width = 4, offset = 1, plot_clear_growth_percents_button),
                                                    column(width = 3, offset = 1, export_growth_percents_button)
                                                  ))),
                              h4(""),
                              fluidRow(
                      ...
                      (
                        ...
                        shinyjs::hidden(div(id = "GrowthPercents_out_Main",
...
tab5 <- tabPanel("Reset/Clear App",
                ...
                tags$head(tags$style(HTML("#textMessageResetApp {font-size: 20px;}")))
                 )
